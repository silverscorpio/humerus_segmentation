...
x_path = os.path.join(code_folder, "data", "data_X")
y_path = os.path.join(code_folder, "data", "data_Y")
paths = (x_path, y_path)
...
# main part which initiates the deep learning pipeline with several options for constants, parameters and hyperparameters
...
X_data, Y_data = pp.get_all_data(
    paths, preprocess=True, expand_dim=True, verbose=False, norm_option="mean_norm"
)
...
trained_model, trained_model_hist = mt.model_train(
...
po.save_model_hist(
    model_params_str, saved_models_path, trained_model, trained_model_hist
)
...
# predictions
test_Y_3d_vals, preds_cat_vals, preds_3d_vals = po.model_predictions(
    model_params_str, trained_model, test_X, test_Y, saved_models_path, make_nifti=True
)

# plot categorical (per class)
po.plot_truth_pred_cat(
    test_X,
    test_Y,
    preds_cat_vals,
    class_ind=2,
...
po.plot_truth_pred_3d(
    test_X, test_Y_3d_vals, preds_3d_vals, sample_ind=0, slice_start=115, slice_end=120
)
