...
def build_unet(
    ...
    dropout: float,
    upconv_type: str,
    encode_filter_size: list,
    decode_filter_size: list,
):
    """
    ...
    if len(encode_filter_size) != 5 or len(decode_filter_size) != 4:
        raise ValueError(
            "incorrect number of values for encoder & decoder blocks (encoder:5, decoder:4)"
        )
    ...
    if upconv_type == "upsampling":
        ...
        u6 = Conv3DTranspose(
            decode_filter_size[0], (2, 2, 2), strides=(2, 2, 2), padding="same"
        )(
            c5
        )  # 128
    u6 = Concatenate()([u6, c4])
    c6 = Conv3D(
        decode_filter_size[0],
        (3, 3, 3),
        activation="relu",
        kernel_initializer=kernel_initializer,
        padding="same",
    ...
    if upconv_type == "upsampling":
        ...
        u7 = Conv3DTranspose(
            decode_filter_size[1], (2, 2, 2), strides=(2, 2, 2), padding="same"
        )(
            c6
        )  # 64
    u7 = Concatenate()([u7, c3])
    c7 = Conv3D(
        decode_filter_size[1],
        (3, 3, 3),
        activation="relu",
        kernel_initializer=kernel_initializer,
        padding="same",
    ...
    if upconv_type == "upsampling":
        ...
        u8 = Conv3DTranspose(
            decode_filter_size[2], (2, 2, 2), strides=(2, 2, 2), padding="same"
        )(
            c7
        )  # 32
    u8 = Concatenate()([u8, c2])
    c8 = Conv3D(
        decode_filter_size[2],
        (3, 3, 3),
        activation="relu",
        kernel_initializer=kernel_initializer,
        padding="same",
    ...
    if upconv_type == "upsampling":
        ...
        u9 = Conv3DTranspose(
            decode_filter_size[3], (2, 2, 2), strides=(2, 2, 2), padding="same"
        )(
            c8
        )  # 16
    u9 = Concatenate()([u9, c1])
    c9 = Conv3D(
        decode_filter_size[3],
        (3, 3, 3),
        activation="relu",
        kernel_initializer=kernel_initializer,
        padding="same",
