...
def save_model_hist(
    model_name: str, save_model_dir: str, train_model, train_model_hist
):
    """
    ...
    with open(
        os.path.join(saved_model_folder_path, (model_name + "_history")), "wb"
    ) as fp:
...
def model_load(model_type: str, model_name: str, save_model_dir: str):
    ...
    if model_type == "unet_w_bbone":
        model_loaded = tf.keras.models.load_model(
            os.path.join(saved_model_folder_path, (model_name + ".h5")),
            custom_objects={
...
def model_plots(model_history, fig_size=(10, 10)):
    ...
    fig, ax = plt.subplots(
        fig_dict[keys.size][0], fig_dict[keys.size][1], figsize=fig_size
    )
    for row in range(0, fig_dict[keys.size][0]):
        for col in range(0, fig_dict[keys.size][1]):
            ax[row, col].plot(
                np.arange(1, (len(model_history.history[keys[0, 0]]) + 1), 1),
                model_history.history[keys[row, col]],
            )
...
def plot_truth_pred_3d(
    test_X, test_Y_3d, preds_3d, sample_ind: int, slice_start: int, slice_end: int
):
    """
