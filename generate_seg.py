...
def export_to_labelmapvol(segmentation_node, ref_vol, labelmap_vol_name):
	new_labelmap_vol_node = slicer.mrmlScene.AddNewNodeByClass('vtkMRMLLabelMapVolumeNode')
	new_labelmap_vol_node.SetName(labelmap_vol_name)
	slicer.modules.segmentations.logic().ExportVisibleSegmentsToLabelmapNode(segmentation_node, 
																		 	 new_labelmap_vol_node, 
																		 	 ref_vol)
	print("Exported to labelmap volume!")
	return new_labelmap_vol_node

def save_node(labelmap_vol, save_filepath):
	slicer.util.saveNode(slicer.util.getNode(labelmap_vol.GetName()), (save_filepath + '.nii'))
...
def main():
	...
	ctscan_folders = natsorted([i for i in os.listdir(default_path) if (not i.endswith('DS_Store')) and (not i.endswith('csv'))])
	for folder_num in ctscan_folders:
		...
		labelmap_vol_node = export_to_labelmapvol(ct_segmentation, ct_vol_node, ct_segmentation_name)
		save_node(labelmap_vol_node, os.path.join(ctvol_path, ct_segmentation_name + '.nii'))
