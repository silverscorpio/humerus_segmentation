...
def import_model_to_segmentation(model, segmentation_node):
	import_bool = slicer.modules.segmentations.logic().ImportModelToSegmentationNode(slicer.util.getNode(model.GetName()), segmentation_node)
	if import_bool:
...
def export_segmentation_to_model(segmentation_node, model_folder_name):
	...
	exportFolderItemId = shNode.CreateFolderItem(shNode.GetSceneItemID(), model_folder_name)
...
def group_colors(segmentation_node, stl_files, color):
	...
	segmentation_node.GetSegmentation().GetSegment(segment_name).SetColor(color_vals[color][0], color_vals[color][1], color_vals[color][2])

	target_seg_id = segmentation_node.GetSegmentation().GetSegmentIdBySegmentName(segment_name)
	seg_edit_node.SetSelectedSegmentID(target_seg_id)

	for i in stl_files:
		effect.setParameter("ModifierSegmentID", segmentation_node.GetSegmentation().GetSegmentIdBySegmentName(i.split('.')[0]))
		effect.self().onApply()
...
def main(color:str):
	...
	ctscan_folders = [i for i in os.listdir(default_path) if (not i.endswith('DS_Store')) and (not i.endswith('csv'))]
	global color_vals
	color_vals = {'blue': (0, 0, 1), 'green': (0, 1, 0), 'red': (1, 0, 0), 'yellow': (1, 1, 0)}
	...
	for folder_num in ctscan_folders:
		...
		stl_files = [i for i in os.listdir(ctvol_path) if i.endswith('.stl') and color in i]
		...
		if len(stl_files) == 0:
			...
			tgt_name = os.path.join(default_path, folder_num, (folder_num + color + '.stl'))
			...
			save_model(os.path.join(ctvol_path, ct_segmentation_name + '.stl'), color)
